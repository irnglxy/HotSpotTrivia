...
export default function MasterHostDashboard() {
  ...
  const saveGame = async () => {
    ...
    if (gameType === 'timeline' && questions.some((question) => question.timelineItems?.length !== 6 || question.timelineItems.some((item) => !item.trim()) || new Set(question.timelineItems.map((item) => item.trim().toLowerCase())).size !== 6 || !question.timelineTopLabel?.trim() || !question.timelineBottomLabel?.trim())) {
      alert('Add six different timeline items and both ordering labels to every question.');
      return;
    }
  ...
  return (
    <div className="min-h-screen bg-zinc-950 text-zinc-100 p-6 md:p-12">
      <div className="max-w-5xl mx-auto">
          ...
          <div className="text-center">
            ...
            <div className="flex justify-between items-center mb-6">
              ...
              <div className="bg-purple-900/50 border border-purple-700 text-purple-200 px-4 py-1.5 rounded-full font-bold text-sm">
                {currentQuestion.gameType === 'word-scramble' ? `Words Found: ${answerStats.totalAnswers}` : `Locked In: ${answerStats.totalAnswers} / ${answerStats.totalPlayers}`}
              </div>
            </div>
            
            {currentQuestion.gameType !== 'simon-says' && currentQuestion.gameType !== 'word-scramble' && <h1 className="text-4xl md:text-5xl font-black text-white mb-12 bg-zinc-900 border border-zinc-800 p-8 rounded-3xl shadow-2xl leading-tight">
            ...
            ) : currentQuestion.gameType === 'pitch-meeting' ? <div className="mb-10 bg-sky-950/50 border border-sky-700 rounded-2xl p-8 text-center"><p className="text-2xl font-black text-sky-200">Pitch Meeting</p><p className="text-zinc-400 mt-2">Players are allocating {currentQuestion.pitchPoints} points between the two pitches.</p></div> : currentQuestion.gameType === 'timeline' ? <div className="mb-10 bg-violet-950/50 border border-violet-700 rounded-2xl p-8 text-center"><p className="text-3xl font-black text-violet-200">Timeline</p><p className="text-zinc-300 mt-3">Players are arranging six items from <span className="font-bold text-violet-200">{currentQuestion.timelineTopLabel}</span> to <span className="font-bold text-violet-200">{currentQuestion.timelineBottomLabel}</span>.</p></div> : <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-10">
            ...
            {currentQuestion.gameType !== 'word-scramble' && answerStats.totalPlayers > 0 && answerStats.totalAnswers === answerStats.totalPlayers && timeLeft > 0 && (
            ...
            >
              {currentQuestion.gameType === 'word-scramble' ? (timeLeft <= 0 ? 'Show Word Recap' : 'End Round Early & Show Recap') : (timeLeft <= 0 ? 'Reveal How Everyone Answered' : 'End Question Early & Reveal Answers')}
            </button>
          </div>
        )}
        ...
        {view === 'answer-reveal' && answerBreakdown?.gameType === 'timeline' && (
          ...
          <div>
            ...
            {questions.map((q, qIndex) => (
              <div key={qIndex} className="mb-6 p-6 bg-zinc-900 border border-zinc-800 rounded-2xl shadow-xl">
                ...
                {gameType !== 'simon-says' && gameType !== 'player-picker' && gameType !== 'word-scramble' && <input
                  type="text" 
