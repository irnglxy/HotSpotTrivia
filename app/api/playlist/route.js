...
export async function POST(request) {
  ...
  try {
    ...
    const insertQuestion = db.prepare('INSERT INTO questions (game_id, question_text, option_a, option_b, option_c, option_d, correct_answer, correct_number, answer_min, answer_max, answer_step, scoring_margin, herd_mode, simon_sequence, autocomplete_answers, scramble_letters, pitch_points, timeline_items, timeline_top_label, timeline_bottom_label, time_limit) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)');
    db.transaction(() => playlist.games.forEach((game, gameIndex) => {
      ...
      (game.questions || []).forEach((question) => insertQuestion.run(gameId, question.question_text, question.option_a, question.option_b, question.option_c, question.option_d, question.correct_answer, question.correct_number, question.answer_min, question.answer_max, question.answer_step, question.scoring_margin, question.herd_mode || 'most', question.simon_sequence, question.autocomplete_answers, question.scramble_letters, question.pitch_points, question.timeline_items, question.timeline_top_label, question.timeline_bottom_label, question.time_limit || 15));
    }))();
