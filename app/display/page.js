...
export default function DisplayScreen() {
  ...
  useEffect(() => {
    ...
    socket.on('question-time-up', () => setTimeLeft(0));
  ...
  return (
    <main className="min-h-screen bg-zinc-950 text-white flex flex-col items-center justify-between p-8 md:p-16 select-none overflow-hidden">
      ...
      <div className="w-full max-w-6xl my-auto py-8">
        ...
        {status === 'answer-reveal' && answerBreakdown?.gameType === 'open-trivia' && (
          <div className="max-w-4xl mx-auto text-center space-y-6"><p className="text-orange-300 font-bold uppercase tracking-widest">Open Trivia</p><h2 className="text-4xl font-black text-white">{answerBreakdown.questionText}</h2><p className="text-zinc-300 text-xl">Correct answer: <span className="font-bold text-emerald-400">{answerBreakdown.correctAnswer}</span></p><div className="max-h-[55vh] overflow-y-auto space-y-3 text-left pr-2">{answerBreakdown.answerCounts.map(({ key, answer, count, isCorrect }) => <div key={key} className={`flex justify-between border rounded-2xl p-5 text-xl ${isCorrect ? 'bg-emerald-950/40 border-emerald-400' : 'bg-zinc-900 border-zinc-800'}`}><span className="font-bold text-white">{answer}{isCorrect && <span className="ml-3 text-emerald-400 text-sm">Accepted</span>}</span><span className="font-mono text-white">{count}</span></div>)}</div></div>
        )}
        ...
        {status === 'answer-reveal' && answerBreakdown?.gameType === 'timeline' && (
          <div className="max-w-4xl mx-auto text-center space-y-6"><p className="text-violet-300 font-bold uppercase tracking-widest">Timeline</p><h2 className="text-4xl font-black text-white">{answerBreakdown.questionText}</h2><div className="rounded-3xl border border-violet-700 bg-violet-950/30 p-6 text-left max-w-2xl mx-auto"><p className="text-center text-sm uppercase tracking-widest text-violet-200 font-bold mb-3">{answerBreakdown.topLabel}</p><div className="space-y-2">{answerBreakdown.correctOrder.map((item, index) => <div key={item} className="flex gap-4 rounded-xl bg-zinc-950 p-4 text-xl"><span className="text-violet-300 font-black">{index + 1}</span><span className="font-bold text-white">{item}</span></div>)}</div><p className="text-center text-sm uppercase tracking-widest text-violet-200 font-bold mt-3">{answerBreakdown.bottomLabel}</p></div><p className="text-zinc-400 font-mono">{answerBreakdown.totalAnswers} / {answerBreakdown.totalPlayers} timelines locked in</p></div>
        )}
